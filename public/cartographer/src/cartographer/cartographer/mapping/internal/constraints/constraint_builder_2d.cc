...
void ConstraintBuilder2D::ComputeConstraint(
  ...
  ceres_scan_matcher_.Match(pose_estimate.translation(), pose_estimate,
                            ...
                            *submap_scan_matcher.grid, &pose_estimate,
                            &unused_summary);
// 再看成本变化：final_cost / initial_cost < 0.1 → 误差下降 90% 以上（效果好）；
      // LOG(INFO) << "test Score=" << score;
      // LOG(INFO) << "test!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! final_cost=" << unused_summary.final_cost;
      // LOG(INFO) << "test:" << match_full_submap;
//add by ahangin ========================================================================================
    // if ( match_full_submap ) {   // 
      // LOG(INFO) << "#################!!!!!!!!!!!!!!!!!!!!!!!!!!! Score=" << score;
      // LOG(INFO) << "condtraint_builder!!!!!!! final_cost=" << unused_summary.final_cost/unused_summary.initial_cost;
    
      if (initialization_callback_) {
        // initialization_callback_(score);  // 通知外部
        initialization_callback_(score);  //final_cose非常小 <0.1匹配成功
      }
// }                          
  //============================================================================= 
...
// add by ahangin========================================================================
void ConstraintBuilder2D::SetInitializationCallback(
    const std::function<void(float /*score*/)> &callback) {
      //  LOG(INFO) << "[ConstraintBuilder2D] SetInitializationCallback called!";
