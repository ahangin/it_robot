...
#include <tf2/utils.h>
#include <std_msgs/Float32.h>   // 加上这一行
...
Node::Node(
  ...
  if (collect_metrics) {
    ...
    // 在 node 初始化早期创建 publisher（确保在注册回调前）
  relocal_pub_ = node_handle_.advertise<std_msgs::Float32>(
      "/relocalization_result", 1, true); // latched = true
  auto_relocalizer_pub_ = node_handle_.advertise<std_msgs::Float32>(
      "/auto_relocalizer_score", 1, true); // latched = true
...
void Node::PublishConstraintList(const ::ros::WallTimerEvent&) {
  if (constraint_list_publisher_.getNumSubscribers() == 0 &&
      relocal_pub_.getNumSubscribers() == 0) return;

  absl::MutexLock lock(&mutex_);
  auto constraint_list = map_builder_bridge_.GetConstraintList();

  constraint_list_publisher_.publish(constraint_list);

  // ----------- 检测跨轨迹约束（非空）-----------
  bool relocal_success = false;
  for (const auto& m : constraint_list.markers) {
    if (m.ns == "Inter constraints, different trajectories" &&
        m.points.size() >= 2) {
  ...
  msg.data = relocal_success ? 1.0f : 0.0f;
  relocal_pub_.publish(msg);

  // ----------- 仅首次输出日志 -----------
  if (relocal_success && !relocalized_) {
    relocalized_ = true;
...
int Node::AddTrajectory(const TrajectoryOptions& options) {
  ...
  // add by ahangin ==================================================================================
  // 获取 pure_localization 参数
  bool pure_localization = false;
  ::ros::param::get("/pure_localization", pure_localization);
  if (pure_localization) {
    auto* pose_graph = map_builder_bridge_.map_builder()->pose_graph();
    auto* pose_graph_2d = dynamic_cast<cartographer::mapping::PoseGraph2D*>(pose_graph);
    if (pose_graph_2d) {
      pose_graph_2d->GetConstraintBuilder()->SetInitializationCallback(
          [this](float score ) {
          // ROS_INFO("Initialization and relocalization! score = %.6f,!!!!!!!!!!!!!!!!!!!!!", score);
          std_msgs::Float32 msg;
          msg.data = score;
          auto_relocalizer_pub_.publish(msg);  // publish 在线程中调用是线程安全的
          });
    } else {
      ROS_ERROR("Initialization and relocalization     failed !!!!!!!!!!!!!!!!!!!!!!!!");
    }
  }
 //  ==================================================================================
