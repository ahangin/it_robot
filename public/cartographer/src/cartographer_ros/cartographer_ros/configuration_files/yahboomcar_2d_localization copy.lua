...
TRAJECTORY_BUILDER_2D.min_range = 0.2            --0.4
TRAJECTORY_BUILDER_2D.max_range = 20.  --20.0
...
TRAJECTORY_BUILDER_2D.ceres_scan_matcher.occupied_space_weight = 50. --1.
TRAJECTORY_BUILDER_2D.ceres_scan_matcher.translation_weight = 1. --10.
TRAJECTORY_BUILDER_2D.ceres_scan_matcher.rotation_weight = 75. ---40.
...
TRAJECTORY_BUILDER_2D.adaptive_voxel_filter.max_length = 0.9 -- 0.5
TRAJECTORY_BUILDER_2D.adaptive_voxel_filter.min_num_points = 100 -- 200
TRAJECTORY_BUILDER_2D.adaptive_voxel_filter.max_range = 50. -- 50.
TRAJECTORY_BUILDER_2D.loop_closure_adaptive_voxel_filter.max_length = 1.5 -- 0.9.
...
TRAJECTORY_BUILDER_2D.real_time_correlative_scan_matcher.angular_search_window = math.rad(40.)-- 20
...
TRAJECTORY_BUILDER_2D.submaps.num_range_data = 200. --90.
--------------------------------------------------------------------------------
POSE_GRAPH.optimize_every_n_nodes = 20. --90  
POSE_GRAPH.constraint_builder.sampling_ratio = 0.15 -- 0.3  --0.3 对局部子图进行回环检测时的计算频率, 数值越大, 计算次数越多
POSE_GRAPH.global_sampling_ratio = 0.003 -- 0.003  ---- 纯定位时候查找回环的频率

POSE_GRAPH.constraint_builder.max_constraint_distance = 15. -- 15. 对局部子图进行回环检测时能成为约束的最大距离
POSE_GRAPH.constraint_builder.min_score = 0.4 -- 0.55  对局部子图进行回环检测时的最低分数阈值
POSE_GRAPH.constraint_builder.global_localization_min_score = 0.5 -- 0.6 
POSE_GRAPH.global_constraint_search_after_n_seconds = 30. --10   纯定位时多少秒执行一次全子图的约束计算

POSE_GRAPH.constraint_builder.fast_correlative_scan_matcher.linear_search_window = 3. -- 7
POSE_GRAPH.constraint_builder.fast_correlative_scan_matcher.angular_search_window = math.rad(10.) -- 30
